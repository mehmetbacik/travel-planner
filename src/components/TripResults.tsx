      ...
      >
        <div className="bg-white rounded-xl shadow-xl overflow-hidden">
          ...
          <div className="p-8 border-b">
            <h2 className="text-2xl font-semibold mb-4">{dict.features.weather}</h2>
            <div className="bg-blue-50 p-4 rounded-lg">
              <p className="text-lg mb-2">{tripPlan.itinerary.weatherForecast.summary}</p>
              <p className="text-gray-600">
                Temperature: {tripPlan.itinerary.weatherForecast.temperature.min}°C -{' '}
                {tripPlan.itinerary.weatherForecast.temperature.max}°C
              </p>
              <p className="text-gray-600">
                Precipitation: {tripPlan.itinerary.weatherForecast.precipitation}
              </p>
            </div>
          </div>
          ...
          <div className="p-8">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              {/* Restaurants */}
              <div>
                <h3 className="text-xl font-semibold mb-4">{dict.features.food}</h3>
                <div className="space-y-4">
                  {tripPlan.itinerary.recommendations.restaurants.map((restaurant, index) => (
              ...
              {/* Attractions */}
              <div>
                <h3 className="text-xl font-semibold mb-4">{dict.features.attractions}</h3>
                <div className="space-y-4">
                  {tripPlan.itinerary.recommendations.attractions.map((attraction, index) => (
                    <div key={index} className="bg-gray-50 p-4 rounded-lg">
                      <p className="font-medium">{attraction.name}</p>
                      <p className="text-sm text-gray-600">{attraction.type}</p>
                      <p className="text-sm text-gray-600">Rating: {attraction.rating}/5</p>
                    </div>
                  ))}
                </div>
              </div>
